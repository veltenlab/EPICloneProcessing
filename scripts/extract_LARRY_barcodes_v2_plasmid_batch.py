...
parser.add_argument('--barcode_extension', help='A sample specific Cell barcode extension for batch processing')
...
no_fastq = []
no_abundant_bcs = []
larry_tmp = args.output + '/LARRY_tmp_' + args.barcode_extension + '/'
...
for i,ff in enumerate(sorted_fastqs):
    ...
    os.system('zcat ' + ff + ' | grep ' + gfp_seq + ' -B 1 >> '+ larry_tmp + 'BC_' +repr(i)+'.txt')
...
for f in os.listdir(larry_tmp):
    ...
    for xx in open(larry_tmp + f).read().strip('\n').split('--'):
        ...
        if len(gfp_bc) >= 34:
            ...
            if is_valid(gfp_bc):
                if cell_bc in ab_bc:
                    tag = '>'+cell_bc+'_'+args.barcode_extension
                    out.write((tag+'\n').encode('utf-8'))
                    out.write((gfp_bc+'\n').encode('utf-8'))
